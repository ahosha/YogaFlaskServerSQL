...
class RecentEvents(Resource):
    ...
    def get(self):
        try:
            return onGetRequest(request)
        except:
            return recent_events_log_err("GET")
...
def recent_events_route(req):
    if req.method == 'GET':
        try:
            ...
            return recent_events_log_err("POST")
...
def onGetRequest(req):
    ...
    ubus_attributes_dict = parse_interface_name(ubus_attributes_dict , sys_up_time)
...
def parse_interface_name(ubus_attr , sys_up_time):
    # Convert values in dictionary
    # Need to parse interface name
    ubus_attributes_dict = ubus_attr
...
def recent_events_log_err(methodType):
    radlogger.log('recent_events_route ' + methodType + 'method.', sys.exc_info())
    response_json = wrap_data({}, error="Error occurred")
    return response_json
