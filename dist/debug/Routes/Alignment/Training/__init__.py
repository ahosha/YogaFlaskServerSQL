...
def training_alignmentActionInvoker(action, req):
    ...
    if action == 'start-speed-test':
         return mockup.genericPostResponse()

    if action == 'stop-speed-test':
         return mockup.genericPostResponse()
...
def training_fine_alignment(req):
    if req.method == 'GET':
        try:
            ...
            fine_response["currentCbw"] = '20'
            fine_response["frequency"] = '5820'
            fine_response["aggregateCapacity"] = '107'
            ...
            for fine in TRAINING_FINE_ALIGNMENT:
                ...
                fine_response["SectorId"] = TRAINING_INITIAL_DATA['sectorId'] if 'sectorId' in TRAINING_INITIAL_DATA  else "TestSectorId"
                fine_response["hbsIp"] = TRAINING_INITIAL_DATA['HbsIp'] if 'HbsIp' in TRAINING_INITIAL_DATA  else "HbsIp"
                fine_response["hbsName"] = TRAINING_INITIAL_DATA['HbsName'] if 'HbsName' in TRAINING_INITIAL_DATA  else "HbsName"
                fine_response["hbsLocation"] = TRAINING_INITIAL_DATA['HbsLocation'] if 'HbsLocation' in TRAINING_INITIAL_DATA  else "HbsLocation"
...
def training_evaluation_results(req):
    if req.method == 'GET':
        ...
        temp_response['CurrentRateIdx'] = '700'
        temp_response['hbsModulationRate'] = 'Training-hbsModulationRate'
...
def init_training_scenario():
    ...
    if not WORKING_MODE == 'TrainingConfig':
        response_json = wrap_data({}, error="Error occurred")
        return response_json
...
def init_training_objects(data):
    ...
    if 'HSU' in data:
        ...
        TRAINING_INITIAL_DATA['apiVersion'] = data['HSU']['apiVersion']
        ...
        TRAINING_INITIAL_DATA['macAddress'] = data['HSU']['macAddress']
        TRAINING_INITIAL_DATA['sectorId'] = data['HSU']['sectorId']
        TRAINING_INITIAL_DATA['antennaConnectionType'] = data['HSU']['antennaConnectionType']
        TRAINING_INITIAL_DATA['antennaGain'] = data['HSU']['antennaGain']
        TRAINING_INITIAL_DATA['cableLoss'] = data['HSU']['cableLoss']
        TRAINING_INITIAL_DATA['minAntennaGain'] = data['HSU']['minAntennaGain']
        TRAINING_INITIAL_DATA['maxAntennaGain'] = data['HSU']['maxAntennaGain']
        TRAINING_INITIAL_DATA['HbsIp'] = data['HSU']['HbsIp']
        TRAINING_INITIAL_DATA['HbsName'] = data['HSU']['HbsName']
        TRAINING_INITIAL_DATA['HbsLocation'] = data['HSU']['HbsLocation']
        TRAINING_INITIAL_DATA['bandName'] = data['HSU']['bandName']
        TRAINING_INITIAL_DATA['aggregateCapacity'] = 1 #data['HSU']['HbsName']
        TRAINING_INITIAL_DATA['attachedAntennaIndication'] = 2 # data['HSU']['HbsLocation']
        TRAINING_INITIAL_DATA['hsuAirState'] = 1 #data['HSU']['bandName']
