...
# Spectrum
SPECTRUM_RANGE_URL = 'range'
START_SPECTRUM_URL = 'start'
STOP_SPECTRUM_URL = 'stop'
SPECTRUM_TABLE_URL = 'table'

class Spectrum(Resource):
    @jwt_required()
    def get(self , op_name):
        name = op_name
        if name == SPECTRUM_RANGE_URL:
            return spectrum_range(request)

        if name == SPECTRUM_TABLE_URL:
            return spectrum_table(request)


    @jwt_required()
    def post(self , op_name):
        name = op_name
        if name == START_SPECTRUM_URL:
            return start_spectrum(request)

        if name == STOP_SPECTRUM_URL:
            return stop_spectrum(request)
