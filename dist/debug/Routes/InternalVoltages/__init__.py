...
def internalVoltages_route(req):
    # Load mapping JSON file
    data = load_mapping_json_file(os.path.join(__location__, 'internalvoltages.json'))
    ...
    if req.method == 'POST':
        try:
            ...
            radlogger.log('battery_route POST method.', sys.exc_info())
            response_json = wrap_data({}, error="Error occurred")
            return response_json
