...
@app.route(consts.PORT_URL, methods=['GET'])
#@jwt_required()
def post_route():
        return jsonify(mockedDataRequest(request, 'port'))

@app.route(consts.BATTERY_URL, methods=['GET'])
#@jwt_required()
def battery_route():
        return jsonify(mockedDataRequest(request, 'battery'))

@app.route(consts.SENSOR_URL, methods=['GET'])
#@jwt_required()
def sensor_route():
        return jsonify(mockedDataRequest(request, 'sensor'))
