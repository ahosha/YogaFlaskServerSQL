...
#CommandStr = " ubus -v list"
#f = os.popen(CommandStr)
#out = f.read()
         
#if "SmartNode" not in out:
#    return


if os.name == 'posix' and USE_UBUS_C == True:
# app.config['USE_UBUS_C']:
    import ubus as ubus
    if DEVICE_TYPE  == 'SmartNode':
        count = 0
        maxcount = 1000
        while (count < maxcount):
            time.sleep(1)
            print 'looking for Smart Node ', count
            CommandStr = " ubus -v list"
            f = os.popen(CommandStr)
            out = f.read()
            if "SmartNode" in out:
                ...
                count = count + 1 
        ubus.InitializeUBusClient("SmartNode")
    else:
...
def linux_ubusc_interface(action, attributes):
    try:
        ...
        elif PRINT_RESPONSES and 'hsuLinkState' not in str(ubus_response) and 'realTimeAndDate' not in str(ubus_response):
            radlogger.log(ubus_response + '\n\n', None)
...
def windows_http_interface_debug(action, attributes):
    ...
    try:
        ...
        for obj in attributes:
            ...
            if (not data['data'] and 'error' in data and 'message' in data['error']):
